...
5	InternetService_Fiber optic	0.0452
6	PaymentMethod_Electronic check	0.0349
7	OnlineSecurity_Yes	0.0319
8	OnlineSecurity_No internet service	0.0307
9	Contract_One year	0.0289
10	InternetService_DSL	0.0250
11	StreamingMovies_Yes	0.0247
12	tenure	0.0197
...
You can use this structure on GitHub:

Customer-Churn-Prediction/
│
├── Customer Churn Analysis Userinput.ipynb
├── Customer_data.csv
├── README.md
│
├── images/
...
│
└── requirements.txt
⭐ A few important corrections before you publish this README

I noticed three things in your notebook that I recommend fixing before putting the project on GitHub:

Your README should report the actual final XGBoost results of 77.00% accuracy, 62.15% F1 and 84.08% ROC-AUC, not the earlier 78%/62% numbers from the preliminary/tuning notes. The final test-set table is the authoritative result.

Your new-customer example enters:

Payment Method: Credit

but your dataset category is Credit card (automatic). Because your encoder uses handle_unknown='ignore', "Credit" can silently be treated as an unknown category. For a reliable prediction demo, enter the exact dataset category:

Credit card (automatic)
In your EDA, statements such as "senior citizens are more likely to churn" and "fiber customers churn more" should ideally be supported with churn rates (%), not just counts. Your README can retain these as observed patterns, but for a stronger portfolio project, add percentage-based churn-rate charts.
